...
- (void)testNsCDNUnion
{
    NSString * random = [NSString stringWithFormat:@"%d",(arc4random() % 1000) + 1];
    NSTimeInterval time = [[NSDate date] timeIntervalSince1970];
    long long int date = (long long int)time;
    NSString * nowTime = [NSString stringWithFormat:@"%lld000",date];
    NSURL * url = [NSURL URLWithString:[NSString stringWithFormat:@"http://7563614540466%@.testns.cdnunion.net/?callback=jQuery18102853321498259902_1442981784438&_=%@",random,nowTime]];
    ...
    [NSURLConnection sendAsynchronousRequest:request
                           ...
                           completionHandler:^(NSURLResponse *response, NSData *data, NSError *error){
                               if (error) {
                                   ...
                                   [_delegate appendRouteLog:responseString];
                               }
                               isrunning = false;
